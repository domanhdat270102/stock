...
interface SaveBtnProps {
    data: CryptoData; // Define the 'data' prop
  }

const SaveBtn: React.FC<SaveBtnProps> = ({ data }) => {
    ...
    const handleClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
      ...
      if (saveCoin) {
        saveCoin(data.id)
        if (allCoins?.includes(data.id) && removeCoin) {
          removeCoin(data.id)
        }
      }
    }
    return (
      <button className="outline-0 bg-none cursor-pointer border-0" onClick={(e) => handleClick(e)}>
        <svg
          className={`w-[1.5rem] ml-1.5 ${allCoins?.includes(data.id)? 'fill-cyan': 'fill-gray-100'}`}
          width="30"
          height="30"
          viewBox="0 0 30 30"
          fill="cyan"
  ...
  return (
    <section className="w-[80%] mx-auto h-full flex flex-col mt-16 mb-24 relative">
      ...
      <button
        ...
        className="w-[2rem] ml-4 hover:scale-110 transition-all ease-in absolute right-0 -top-10"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="1em"
